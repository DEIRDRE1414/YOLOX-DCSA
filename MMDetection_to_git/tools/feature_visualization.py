    ...
    if isinstance(features, torch.Tensor):
        for heat_maps in features:
            ...
            for heatmap in heatmaps:
                ...
                img = mmcv.imread('demo/2243.png')
                heatmap = cv2.resize(heatmap, (img.shape[1], img.shape[0]))
                superimposed_img = cv2.addWeighted(heatmap, 0, img, 1, 0)
        ...
        for featuremap in features:
            ...
            for heatmap in heatmaps:
                ...
                #img = mmcv.imread('demo/2243.png')
                #heatmap = cv2.resize(heatmap, (img.shape[1], img.shape[0]))
                #superimposed_img = cv2.addWeighted(heatmap, 0.5, img, 0.3, 0)
                heatmap = cv2.applyColorMap(heatmap, cv2.COLORMAP_JET)
                img = mmcv.imread('demo/5405.png')
                heatmap = cv2.resize(heatmap, (img.shape[1], img.shape[0]))
                superimposed_img = cv2.addWeighted(heatmap, 0.3, img, 1, 0)
