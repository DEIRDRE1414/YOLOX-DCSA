...
class Mosaic(BaseTransform):
    ...
    Args:
        ...
        prob (float): Probability of applying this transformation.应用这个变换的概率。
            Defaults to 1.0.
    """

    def __init__(self,
                 img_scale: Tuple[int, int] = (640, 640),
                 center_ratio_range: Tuple[float, float] = (0.5, 1.5),             #中心比范围
                 bbox_clip_border: bool = True,                                    #bbox 剪辑边框
                 pad_val: float = 114.0,
                 prob: float = 1.0) -> None:
    ...
    def get_indexes(self, dataset: BaseDataset) -> int:
        """Call function to collect indexes.调用函数收集索引
...
class MixUp(BaseTransform):
    ...
    Args:
        ...
        ratio_range (Sequence[float]): Scale ratio of mixup image.混合图像的比例比
            Defaults to (0.5, 1.5).
        flip_ratio (float): Horizontal flip ratio of mixup image.混合图像的水平翻转比
            Defaults to 0.5.
        pad_val (int): Pad value. Defaults to 114.
...
class RandomAffine(BaseTransform):
    ...
    Args:
        max_rotate_degree (float): Maximum degrees of rotation transform.旋转变换的最大度。
            Defaults to 10.
        max_translate_ratio (float): Maximum ratio of translation.最大平移比。
            Defaults to 0.1.
        scaling_ratio_range (tuple[float]): Min and max ratio of
            scaling transform. Defaults to (0.5, 1.5).缩放变换的最小和最大比率。
        max_shear_degree (float): Maximum degrees of shear
            transform. Defaults to 2.最大剪切变换度。
...
class YOLOXHSVRandomAug(BaseTransform):
    ...
    Args:
        hue_delta (int): delta of hue. Defaults to 5.色调增量
        saturation_delta (int): delta of saturation. Defaults to 30.饱和度增量
        value_delta (int): delat of value. Defaults to 30.价值的延迟
    """
