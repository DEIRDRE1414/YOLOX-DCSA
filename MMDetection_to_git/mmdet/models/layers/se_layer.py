...
class SELayer(BaseModule):
    ...
    Args:
        channels (int): The input (and output) channels of the SE layer.通道（int）：SE层的输入（和输出）通道
        ratio (int): Squeeze ratio in SELayer, the intermediate channel will be
            ``int(channels/ratio)``. Defaults to 16.SELayer 中的压缩比，中间通道将为“int(channels/ratio)”。默认为 16
        conv_cfg (None or dict): Config dict for convolution layer.
            Defaults to None, which means using conv2d.卷积层的配置字典。默认为 None，这意味着使用 conv2d。
        act_cfg (dict or Sequence[dict]): Config dict for activation layer.激活层的配置字典
            If act_cfg is a dict, two activation layers will be configurated
            by this dict. If act_cfg is a sequence of dicts, the first
            activation layer will be configurated by the first dict and the
            second activation layer will be configurated by the second dict.
            Defaults to (dict(type='ReLU'), dict(type='Sigmoid'))如果 act_cfg 是一个字典，则该字典将配置两个激活层。如果 act_cfg 是字典序列，则第一个激活层将由第一个字典配置，第二个激活层将由第二个字典配置。
        init_cfg (dict or list[dict], optional): Initialization config dict.初始化配置字典。默认为无
            Defaults to None
    """
...
class ChannelAttention(BaseModule):
    ...
    def forward(self, x: Tensor) -> Tensor:
        """Forward function for ChannelAttention.用于通道注意的前向函数"""
