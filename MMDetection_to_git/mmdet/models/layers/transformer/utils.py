...
class ConditionalAttention(BaseModule):
    """A wrapper of conditional attention, dropout and residual connection.条件注意、辍学和剩余联系的包装器

    Args:
        embed_dims (int): The embedding dimension.嵌入维度
        num_heads (int): Parallel attention heads.平行注意头
        attn_drop (float): A Dropout layer on attn_output_weights.属性输出权重上的丢弃层
            Default: 0.0.
        proj_drop: A Dropout layer after `nn.MultiheadAttention`.`nn.多头注意后的丢弃层
            Default: 0.0.
        cross_attn (bool): Whether the attention module is for cross attention.注意模块是否用于交叉注意
            Default: False
        keep_query_pos (bool): Whether to transform query_pos before cross
            attention.是否在交叉注意前转换查询位置
            Default: False.
    ...
    def forward_attn(self,
        ...
        """Forward process for `ConditionalAttention`.条件注意的前向处理

        Args:
            query (Tensor): The input query with shape [bs, num_queries,
                embed_dims].带有Shape的输入查询
...
class MLP(BaseModule):
    ...
    used in DETR series detectors.带有RELU的非常简单的多层感知器(也称为FFN)。主要用于DETR系列探测器

    Args:
        input_dim (int): Feature dim of the input tensor.输入张量的特征尺寸
        hidden_dim (int): Feature dim of the hidden layer.隐藏层的要素暗淡
        output_dim (int): Feature dim of the output tensor.输出张量的特征尺寸
        num_layers (int): Number of FFN layers. As the last
            layer of MLP only contains FFN (Linear).FFN层数。因为MLP的最后一层只包含FFN(线性)
    """
...
class DynamicConv(BaseModule):
    """Implements Dynamic Convolution.实现动态卷积

    This module generate parameters for each sample and
    use bmm to implement 1*1 convolution.该模块为每个样本生成参数，并使用bmm进行1*1卷积。代码修改自`官方GitHub资源库 Code is modified
