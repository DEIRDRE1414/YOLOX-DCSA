...
class YOLOXHead(BaseDenseHead):
    ...
    Args:
        ...
        feat_channels (int): Number of hidden channels in stacking convs.堆叠卷积中的隐藏通道数
            Defaults to 256
        stacked_convs (int): Number of stacking convs of the head.头部的堆叠转换数。默认为 (8, 16, 32)。
            Defaults to (8, 16, 32).
        strides (Sequence[int]): Downsample factor of each feature map.每个特征图的下采样因子。默认为无
             Defaults to None.
        use_depthwise (bool): Whether to depthwise separable convolution in
            blocks. Defaults to False.是否以块为单位进行深度可分离卷积。默认为 False。
        dcn_on_last_conv (bool): If true, use dcn in the last layer of
            towers. Defaults to False.如果为 true，则在最后一层塔中使用 dcn。默认为 False。
        conv_bias (bool or str): If specified as `auto`, it will be decided by
            the norm_cfg. Bias of conv will be set as True if `norm_cfg` is
            None, otherwise False. Defaults to "auto".如果指定为“auto”，则由norm_cfg决定。如果“norm_cfg”为 None，则 conv 的偏差将设置为 True，否则为 False。默认为“自动”
        conv_cfg (:obj:`ConfigDict` or dict, optional): Config dict for
            convolution layer. Defaults to None.卷积层的配置字典。默认为无。
        norm_cfg (:obj:`ConfigDict` or dict): Config dict for normalization
            layer. Defaults to dict(type='BN', momentum=0.03, eps=0.001).规范化层的配置字典。默认为 dict(type=&#39;BN&#39;, Momentum=0.03, eps=0.001)
        act_cfg (:obj:`ConfigDict` or dict): Config dict for activation layer.
            Defaults to None.激活层的配置字典。默认为无。
        loss_cls (:obj:`ConfigDict` or dict): Config of classification loss.分类损失的配置
        loss_bbox (:obj:`ConfigDict` or dict): Config of localization loss.定位丢失的配置
        loss_obj (:obj:`ConfigDict` or dict): Config of objectness loss.对象性损失的配置
        loss_l1 (:obj:`ConfigDict` or dict): Config of L1 loss.L1损耗配置
        train_cfg (:obj:`ConfigDict` or dict, optional): Training config of
            anchor head. Defaults to None.锚头的训练配置。默认为无。
        test_cfg (:obj:`ConfigDict` or dict, optional): Testing config of
            anchor head. Defaults to None.测试锚头的配置。默认为无
        init_cfg (:obj:`ConfigDict` or list[:obj:`ConfigDict`] or dict or
            list[dict], optional): Initialization config dict.
            Defaults to None.初始化配置字典。默认为无。
    """
    ...
    def __init__(
        ...
        #self.dcn_on_last_conv = Dynamic_Conv
    ...
    def _init_layers(self) -> None:
        """Initialize heads for all level feature maps.初始化配置字典。默认为无。"""
